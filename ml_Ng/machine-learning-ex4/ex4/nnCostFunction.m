...
cost = -y_matrix .* log(sig) - (1-y_matrix).*log(1-sig);

t1 = Theta1(:, 2:end);
t2 = Theta2(:, 2:end);
J = 1/m*sum(sum(cost)) + (lambda/2/m) * ( sum(sum(t1.^2)) + sum(sum(t2.^2)) );
...
Theta1 = [zeros(size(Theta1, 1), 1) t1];
Theta2 = [zeros(size(Theta2, 1), 1) t2];
